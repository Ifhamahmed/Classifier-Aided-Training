...
class Dataloader(object):
    ...
    def __init__(self, image_paths, mask_paths, vector_paths, image_size, numclasses, channels=None, palette=None, seed=None):
        if channels is None:
            channels = [3, 12]
    ...
    def _get_image_mask_label(self, index1, index2, index3, should_augment=False):
        image_path = self.image_paths
        mask_path = self.mask_paths
        vector_path = self.vector_paths
        ...
        df = pd.read_csv(vector_path + '/' + index3, sep=',', header=None)
        lbl = df.values[:, 1]
        o_lbl = lbl
        ...
        return img, mask, lbl#, o_img, o_mask
    ...
    def data_gen(self, should_augment=False, batch_size=4):
        ...
        s = sorted(os.listdir(vector_path))
        n, m, s = shuffle(n, m, s)
        while True:
            image = np.zeros((batch_size, sz[0], sz[1], 3)).astype("float32")
            ohm = np.zeros((batch_size, sz[0], sz[1], self.numClasses)).astype("float32")
            labels = np.zeros((batch_size, self.numClasses)).astype('float32')

            for i in range(c, c + batch_size):
                aug_img, aug_mask, aug_label = self._get_image_mask_label(n[i], m[i], s[i], should_augment=should_augment)
                ...
                ohm[i - c] = train_mask
                labels[i - c] = aug_label

            c += batch_size
            if c + batch_size >= len(os.listdir(image_path)):
                c = 0
            yield image, ohm, labels#, mask, o_image, o_mask
...
#                   vector_paths='/home/ifham_fyp/PycharmProjects/dataset/train_vectors/train',
#                   image_size=[512, 512],
#                   numclasses=29,
#                   channels=[3, 3],
#                   palette=palette,
